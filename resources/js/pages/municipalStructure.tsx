            ...
            <section className="bg-[#1F4737] py-16 text-[#F3EEE2]">
                <div className="mx-auto max-w-7xl px-6">
                    ...
                    <p className="mt-4 max-w-2xl text-sm leading-relaxed text-[#F3EEE2]/80">
                        The Board of the Municipality oversees governance, service delivery, and development across all wards.
                    </p>
                </div>
            </section>
                ...
                <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {boardMembers.map((member) => (
                        <div key={member.id} className="rounded-xl border border-[#1F4737]/10 bg-white p-6 shadow-sm">
                            ...
                            <h3 className="mt-4 text-center font-serif text-base font-semibold text-[#1F4737]">{member.name}</h3>
                            <p className="text-center text-xs font-semibold uppercase tracking-wide text-[#D4A24C]">{member.role}</p>
                            {member.bio && <p className="mt-3 text-center text-sm leading-relaxed text-[#241F1A]/70">{member.bio}</p>}
                        </div>
                    ))}
                </div>
            </section>
                        ...
                        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {wards.map((ward, i) => (
                                <div key={ward.id} className="rounded-xl border border-[#1F4737]/10 bg-white p-6 shadow-sm">
                                    ...
                                    {ward.description && <p className="mt-2 text-sm leading-relaxed text-[#241F1A]/70">{ward.description}</p>}
                                </div>
                            ))}
                        </div>
                    </div>
